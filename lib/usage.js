...
async function apiGet(path, retriesLeft = 1) {
  ...
  try {
    ...
    // one blip should not replace good numbers with an error.
    if (err?.name !== 'TimeoutError' && retriesLeft > 0) {
...
// most this often. On a 10 second refresh interval, re-requesting it every tick
...
async function loadCredits(orgId) {
  if (creditsCache.at && Date.now() - creditsCache.at < CREDITS_TTL_MS) {
  ...
  try {
    ...
    // Accounts with no prepaid credit 404 here. Either way, a failure must not
    // take down the usage numbers or blank a balance we already had.
    return creditsCache.value;
  }
}
