...
// Chrome exempts an extension's requests from CORS only while the host
// permission is actually held, and declaring it in the manifest is not the
// same as holding it — site access can be narrowed to "on click", and from
// then on every call to claude.ai is an ordinary cross-origin request from
// chrome-extension://…, preflighted and refused for want of an
// Access-Control-Allow-Origin header. It never leaves the browser. `fetch`
// reports that as "Failed to fetch", the same string it gives for an
// unplugged cable, so it has to be ruled out explicitly rather than inferred.
export const CLAUDE_ORIGIN_PATTERN = 'https://claude.ai/*';

export async function hasHostAccess() {
  try {
    return await chrome.permissions.contains({ origins: [CLAUDE_ORIGIN_PATTERN] });
  } catch {
    // No extension APIs here (the tests), so assume access rather than
    // manufacture a failure that cannot be true.
    return true;
  }
}

// Known limit buckets, in the order we want to show them. Anything the API
// starts returning that is not listed here still gets rendered once it carries
// usage, just after these and with a label derived from the key.
...
async function apiGet(path, { retry = false } = {}) {
  ...
  try {
    resp = await fetch(`${API_BASE}${path}`, {
      ...
      headers: {
        accept: '*/*',
        // No content-type: a GET carries no body to describe, and the header
        // is not on the CORS safelist, so sending it turned every request into
        // a preflighted one for nothing.
...
//
// Unrecognised buckets have to carry usage to appear at all. The API ships
// codenamed buckets for models and promotions before they are announced —
// `tangelo`, `omelette_promotional`, `nimbus_quill` — normally as null, but an
// account enrolled in one gets a live entry sitting at zero instead. Rendered,
// that is a card named after something the reader has never heard of, showing
// no usage, never moving and impossible to dismiss. A bucket we can name still
// shows at 0%, because a quiet session or weekly limit is a real reading about
// a limit you know you have; an unnamed one earns its place by being used.
export function toLimits(data) {
  ...
  for (const key of ordered) {
    ...
    if (!LIMIT_LABELS[key] && utilization <= 0) continue;
    limits.push({
      key,
      label: labelFor(key),
      utilization,
...
// A balance of nothing is not a reading. Every account that has never bought
// credit holds exactly $0 and always will, so the line reporting it never
// changes and never says anything — and on an account with no spend limit
// either, that empty line was the whole card.
export function hasSpendableCredit(credit) {
  return Number.isFinite(credit?.balanceCents) && credit.balanceCents > 0;
}

// The monthly limit is a ceiling you set; the balance is the money that pays
// for the spend underneath it. They are set independently, so the limit is not
// always the thing that stops you — and when it is not, measuring against it
// tells you the opposite of what you need to know. $48.69 spent against an $80
// limit with $12.31 of credit left read as "61% used" and offered $80 as the
// ceiling, when that account can only reach $61 and was four fifths of the way
// there. Whichever runs out first is the one the card should be about.
export function spendCeiling(extra) {
  const usedCents = extra?.usedCents;
  const limitCents = extra?.totalCents;
  if (!Number.isFinite(usedCents) || !Number.isFinite(limitCents) || limitCents <= 0) return null;

  // What the credit allows in total: what has gone already plus what is left.
  const affordableCents = hasSpendableCredit(extra) ? usedCents + extra.balanceCents : null;
  const boundByCredit = affordableCents != null && affordableCents < limitCents;
  const ceilingCents = boundByCredit ? affordableCents : limitCents;

  return {
    ceilingCents,
    limitCents,
    boundByCredit,
    utilization: ceilingCents > 0 ? (usedCents / ceilingCents) * 100 : 100
  };
}
...
// The wall-clock time is what you plan around, so it leads; the countdown
// follows in brackets for the "how long have I got" glance. A reset a day or
// more out names the weekday it lands on, since the time alone would not say
// which one.
function formatClock(date, withWeekday = false) {
...
export function formatCountdown(resetIso, now = Date.now()) {
  ...
  if (days > 0) {
    return `Resets on ${formatClock(reset, true)} (${days} ${days === 1 ? 'day' : 'days'})`;
  }

  let left = 'under a minute';
  if (hrs > 0) left = `${hrs}h ${mins}m`;
  else if (mins > 0) left = `${mins}m`;
  return `Resets at ${formatClock(reset)} (${left})`;
}
...
export function describeError(code, detail) {
  switch (code) {
    ...
    case 'NO_ACCESS':
      return {
        icon: '🚫',
        title: 'Site access is switched off',
        desc: 'Chrome is holding back this extension’s access to claude.ai, so its requests are refused before they leave the browser. Grant it below, or set Site access to “On all sites” at chrome://extensions.',
        grant: true
      };
    case 'BLOCKED':
      return {
